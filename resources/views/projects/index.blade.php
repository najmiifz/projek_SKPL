      ...
      @foreach($projects as $project)
        <a href="{{ route('projects.show', $project) }}" class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all hover:-translate-y-1 block group">
          <div class="flex justify-between items-start mb-3">
            ...
            <span class="text-xs text-slate-400">{{ $project->start_date ?? 'TBD' }} - {{ $project->end_date ?? 'TBD' }}</span>
          </div>
          <h3 class="font-bold text-lg text-slate-800 mb-1 group-hover:text-blue-600 transition">{{ $project->name }}</h3>
          <p class="text-slate-500 text-sm mb-4 line-clamp-2">{{ $project->description ?? 'Tidak ada deskripsi.' }}</p>
          ...
          <div class="mt-4 pt-4 border-t flex items-center justify-between text-xs text-slate-500">
